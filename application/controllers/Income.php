...
//-----------------------------------------------------------------------------------------
    public function view() {
        ...
        if (isset($_POST['limit'])) {
            ...
            $limit = ($this->uri->segment(8)) ? $this->uri->segment(8) : $limit = 300;
        }
...
//-----------------------------------------------------------------------------------------
    public function add() {
        ...
        $data[$this->name.'Records'] = $this->model->get_where('tree_account', 'accountno=311');
...
//-----------------------------------------------------------------------------------------
    public function addD() {
        $data['style']      =  $this->style;
        $data['title'] = $this->lang->line('entryD');
        $data['bankRecords'] = $this->model->get_where('tree_account', 'accountno=1111');
        $data['fees'] = $this->model->get_where('tree_account', 'accountno=4111');
        $data[$this->name.'Records'] = $this->model->get_where('tree_account', 'accountno=311');
        if ($this->uri->segment(4) == 'add') {
            $id = $this->uri->segment(3);
            $where = "id = " . $id;
        ...
        $this->load->view('inc/financeDep/'.$this->style.'/header',$this->data_header);
        $this->load->view($this->name.'/sub_menu');
		$this->load->view($this->name.'/addD',$data);
        $this->load->view('inc/financeDep/'.$this->style.'/footer',$this->data_footer);
    }
...
//-----------------------------------------------------------------------------------------
    public function createD() {
        $data['details'] = $this->lang->line('incomeTr')." - ".$this->lang->line('group')." ".$this->lang->line('trainee_s') ." ".$this->lang->line('pointSale');
        $data['thedate'] = $_POST['thedate'];
        $data['thetime'] = date("h:i:s");
        $data['user_id'] = $this->session->userdata('userIdCC');
        $this->model->create('journal', $data);
		//print_r($data);exit;
        $journal_id = $this->db->insert_id();

        $data1['journal_id']    = $journal_id;
        $data1['accountno']     = $_POST['accountno'];
        $data1['box_accountno'] = $_POST['box_accountno'];
        $data1['thedate']       = $_POST['thedate'];
        $data1['byName']         = $this->lang->line('group')." ".$this->lang->line('trainee_s');
        $data1['depositor']      = $this->lang->line('group')." ".$this->lang->line('trainee_s');
        $data1['details']        = $this->lang->line('incomeTr');
        $data1['thevalue']      = $_POST['thevalue1'];
        $this->model->create("income", $data1);

		$data5['journal_id'] = $journal_id;
        $data5['accountno'] = $_POST['accountno'];
        $data5['box_accountno'] = $_POST['box_accountno2'];
        $data5['thedate'] = $_POST['thedate'];
        $data5['thevalue'] = $_POST['thevalue2'];
        $data5['beneficiary'] = $this->lang->line('bank');
        $data5['details'] = $this->lang->line('fees')." ".$this->lang->line('pointSale');
        $this->model->create("expense", $data5);

        $id = $this->db->insert_id();

		
		$data2['journal_id']  = $journal_id;
        $data2['fromaccount'] = $_POST['thevalue1'];
        $data2['toaccount']   = 0;
        $data2['accountno']   = $_POST['box_accountno'];
        $this->model->create('journal_details', $data2);

        $data3['journal_id'] = $journal_id;
        $data3['fromaccount'] = $_POST['thevalue2'];
        $data3['toaccount'] = 0;
        $data3['accountno'] = $_POST['box_accountno2'];
        $this->model->create('journal_details', $data3);
        ...
        <script langauge="javascript" type="text/javascript">
            <!--
                    function goback()
            {
                location = "<?php echo base_url($this->name.'/addD/' . $id . '/addD'); ?>";
            }
            setTimeout('goback()', 1500);
            //-->
        </script>
...
//-----------------------------------------------------------------------------------------
    public function update($id, $journal_id) {
        ...
        $this->load->view('inc/financeDep/'.$this->style.'/header',$this->data_header);
        $data['msg'] = '<div class="update_msg">' . $data['title'] . '</div>';
        $this->load->view('home/msg', $data);
        ?>
        <script langauge="javascript" type="text/javascript">
            <!--
